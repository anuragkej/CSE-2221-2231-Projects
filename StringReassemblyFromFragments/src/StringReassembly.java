...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        for (String s : strSet) {
            if (s.contains(str)) {
                //sets test to false if str is a substring of s
                flag = false;
            }
        }
        if (flag) {
            //creates temporary set to add intended elements
            ...
            tempSet.add(str);
            //updates strSet and clears tempSet
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //iterate while the input does not reach the end of stream
        while (!input.atEOS()) {
            line = input.nextLine();
