...
public class Set3a<T extends Comparable<T>> extends SetSecondary<T> {
    ...
    private static <T extends Comparable<T>> T removeFromTree(BinaryTree<T> t,
        ...
        T label = t.disassemble(tLeft, tRight);
        T removed;

        if (x.compareTo(label) == 0) {
            removed = label;
            if (tRight.size() > 0) {
                T temp = removeSmallest(tRight);
                t.assemble(temp, tLeft, tRight);
            } else {
                t.transferFrom(tLeft);
            }
        } else if (x.compareTo(label) > 0) {
            removed = removeFromTree(tRight, x);
            t.assemble(label, tLeft, tRight);
        } else {
            removed = removeFromTree(tLeft, x);
            t.assemble(label, tLeft, tRight);
        }
        return removed;
    }
