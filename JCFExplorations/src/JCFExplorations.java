        ...
        for (int i = 0; i < mapSize; i++) {
            ...
            if (pair.key().charAt(0) == initial) {
                int value = (int) (pair.value()
                        + (pair.value() * raisePercent * 0.01));
                tempMap.add(pair.key(), value);
            } else {
        ...
        for (java.util.Map.Entry<String, Integer> pair : map.entrySet()) {
            if (pair.getKey().charAt(0) == initial) {
                int value = (int) (pair.getValue()
                        + (pair.getValue() * raisePercent * 0.01));
                pair.setValue(pair.getValue() + (pair.getValue() * value));
            }
        }
