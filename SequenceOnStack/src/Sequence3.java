...
public class Sequence3<T> extends SequenceSecondary<T> {
    ...
    private static <T> void setLengthOfLeftStack(Stack<T> leftStack,
        ...
        int leftStackLength = leftStack.length();
        if (leftStackLength > newLeftLength) {
            leftStack.flip();
            for (int i = 0; i < leftStackLength - newLeftLength; i++) {
                T x = leftStack.pop();
                rightStack.push(x);
            }
            leftStack.flip();
        ...
        if (leftStackLength < newLeftLength) {
            leftStack.flip();
            for (int i = 0; i < newLeftLength - leftStackLength; i++) {
                T x = rightStack.pop();
                leftStack.push(x);
            }
            leftStack.flip();
