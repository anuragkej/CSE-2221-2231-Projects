...
public final class CryptoUtilities {
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber wOrig = new NaturalNumber2(w);
        ...
        result = (wCopy.equals(one)) || (!(w.equals(one)));
        w.transferFrom(wOrig);
        n.transferFrom(nOrig);
        return result;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 1; i < fifty; i++) {
                NaturalNumber test = randomNumber(nCopy);

                while (test.compareTo(one) <= 0 || test.compareTo(nCopy) >= 0) {
                    test = randomNumber(nCopy);
                }
                if (isWitnessToCompositeness(test, n)) {
                    result = false;
                }
            }
